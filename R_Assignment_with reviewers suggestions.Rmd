...
#Load packages to enable working with the data
```{r}
library("tidyverse")
library("dplyr")
library(data.table)
...
#Data Preparation
#Read the data files (fang.mf and snp.mf) we are going to use. Since a header is present we will set the “header” argument to TRUE so that R knows the first line contains a header.
...
```{r}
head(fang.mf) #helps in visualizing the structure of the object
dim(fang.mf) #returns these dimensions: rows (2781) and columns (986)
nrow(fang.mf) #number of rows 2781
ncol(fang.mf) #number of columns 986
...
object.size(fang.mf)#Size of file is 22681376 bytes
```
##Data inspection of snp_position.txt (Now named snp.mf)
```{r}
head(snp.mf) #helps in visualizing the structure of the object
dim(snp.mf) #returns these dimensions: rows (983) and columns (15)
...
object.size(snp.mf)#returns 327392 bytes
```
...
write.csv(snp_extracted,"./snp_extracted_csv.csv",row.names = TRUE)#Recommende by reviewer
View(snp_extracted)
maize <- fang.mf %>% filter(Group == "ZMMIL" | Group == "ZMMLR" | Group == "ZMMMR")
write.table(maize,"./Data_processing/maize.txt")
write.csv(maize,"./maize_csv.csv",row.names = TRUE)#Recommended by reviewer
```
...
write.csv(teosinte,"./teosinte_csv.csv",row.names = TRUE)#Recommended by reviewer
```
...
#Save the joined maize file (maize_final) and view file
```{r}
maize_transposed <- t(maize[,-c(1:3)]) %>% as.data.frame()
write.table(maize_transposed,"./Data_processing/maize_transposed.txt")
write.csv(maize_transposed,"./maize_transposed_csv.csv",row.names = TRUE)#Recommended by reviewer
...
write.csv(maize_final,"./maize_final_csv.csv",row.names = TRUE)#Recommended by reviewer
```
...
#Save the joined teosinte file (teosinte_final)
```{r}
teosinte_transposed <- t(teosinte[,-c(1:3)]) %>% as.data.frame()
write.table(teosinte_transposed,"./Data_processing/teosinte_transposed.txt")
write.csv(teosinte_transposed,"./teosinte_transposed_csv.csv",row.names = TRUE)#Recommended by reviewer
...
write.csv(teosinte_final,"./teosinte_final_csv.csv",row.names = TRUE)#Recommended by reviewer
```
...
for (chr in 1:10){
  ...
  write.csv(maize_chr,file = paste("./Maize_chromosome/maize_chr",chr,"_Q.csv",sep = ""))#Recommended by reviewer
  ...
  write.csv(maize_chr_d,file = paste("./Maize_chromosome/maize_chr",chr,"_H.csv",sep = ""))#Recommended by reviewer
}
```
...
snp.mf %>% 
  ...
  theme_replace() +
  ggtitle("SNP Count vs Chromosome") + #Tittle recommended by reviewer
  ylab('Number of SNPs') +
  xlab('Chromosome') 
ggsave("./Data_visualization/Number of snp_chromosome.png")
```
#1(ii)Plotting SNP density across the chromosome
